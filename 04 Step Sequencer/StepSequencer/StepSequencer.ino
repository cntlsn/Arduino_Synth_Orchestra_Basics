/*
  ...
  DESCRIPTION:
  Reads values from 4 potentiometers to change the pitch for any of the steps of the step sequencer tone generator.
  Control pitch for each step with a potentiometer.
  Control BPM with an LDR.
  Tempo visualfeedback from one LED for each step.
  
  CIRCUIT:
  Connect a speaker or piezo to pin number 12 and to ground;
  Connect one lead of an LDR to 5v, the other to Analog Input pin 4 and to ground through a 10k resistor;
  Connect the first lead of a potentiometer to 5v and the last to ground, the one in the middle to Analog Input pin 0;
  (Repeat for all the four potentiometers connecting the middle lead to Analog Input pins 0, 1, 2, 3);
  Connect the short lead of an LED to ground through a 220 ohm resistor and the long lead to Digital pin 2;
  (Repeat for all the four LEDs connecting the long lead to Digital pins 2, 3, 4, 5);
 
  This example code is for educational purpose and it is in the public domain.

  Author: Alessandro Contini & Paolo Cavagnolo
...
int LDR = A4;                              // LDR pin
int sensorRead;                            // Variable to store the value of the BPM LDR
...
void loop() { 
  ...
  for (k = 0; k <= 3; k++) {                                 // Cycle on each pot
    ...
    value[k] = map(analogRead(k), 0, 1023, 0, 8000);         // Mapping the value of the Potentiometer to have a wider range of values
    ...
    // BPM stuff
    sensorRead = analogRead(LDR);                               // Read from Analog Input 0 to define BPM
